...
export interface TopicIdWeightPair {
  topicId: string;
  weight: string;
}
...
function createBaseTopicIdWeightPair(): TopicIdWeightPair {
  return { topicId: "0", weight: "" };
}

export const TopicIdWeightPair: MessageFns<TopicIdWeightPair> = {
  encode(message: TopicIdWeightPair, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): TopicIdWeightPair {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  fromJSON(object: any): TopicIdWeightPair {
    return {
  ...
  toJSON(message: TopicIdWeightPair): unknown {
    const obj: any = {};
    if (message.topicId !== "0") {
  ...
  create(base?: DeepPartial<TopicIdWeightPair>): TopicIdWeightPair {
    return TopicIdWeightPair.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<TopicIdWeightPair>): TopicIdWeightPair {
    const message = createBaseTopicIdWeightPair();
